...
- âš ï¸ **Log Erasure:** Attackers flush Windows Event Logs (`wevtutil cl`) upon gaining administrative tokens.
- âš ï¸ **Resource Bloat:** Enterprise EDR agents consume over 500MB RAM, causing severe system slowdowns.
- âš ï¸ **System Misconfigurations:** Standard users accidentally apply corrupted ACL rules when attempting manual drive protection.
...
```
+-----------------------------------------------------------------------------------+
|  [ðŸ›¡ï¸] REAL-TIME FILE SYSTEM LOCKING                                              |
|  Direct Win32 ACL injection with (R,D) flags preventing copy, move, or rename.    |
+-----------------------------------------------------------------------------------+
|  [ðŸ”¬] ATTACK FORENSICS & THREAT INTELLIGENCE CENTER                              |
...
+-----------------------------------------------------------------------------------+
|  [ðŸ§©] SHAMIR'S SECRET SHARING                                                     |
|  M-of-N secret shard key reconstruction for administrative disaster recovery.     |
+-----------------------------------------------------------------------------------+
|  [ðŸ›¡ï¸] SECURITY ADVISOR AGENT                                                      |
|  Autonomous security advisor analyzing system events in real-time.               |
+-----------------------------------------------------------------------------------+
```

- ðŸ”‘ **Single-Prompt UAC Self-Elevation:** Checks admin elevation on startup via `net session` and requests `RunAs` elevation once to prevent loop windows.
- ðŸ‘¥ **Role-Based Access Control (RBAC):** Enforces `SuperAdmin`, `Admin`, and `ReadOnly` access tiers across UI and Tauri IPC calls.
- âœ¨ **Transitions-Dev Motion System:** 60fps compositor-friendly modal entrance/exit lifecycle animations (`isOpen` & `isClosing`).
- âš¡ **Emergency Lockdown Mode:** Instant one-click UI trigger for hardware peripheral bus isolation alerts.
...
```mermaid
graph TB
    User["ðŸ‘¤ Security Operator"] <--> Frontend["ðŸ–¥ï¸ React Cyber Command Center UI"]
    Frontend <-->|"Tauri IPC Async Bridge"| Backend["ðŸ¦€ Rust Core System Engine"]
    Backend <--> Auth["ðŸ” RBAC Session & Key Handler"]
    Backend <--> NTFSEngine["ðŸ›¡ï¸ Win32 icacls / attrib ACL Engine"]
    Backend <--> ForensicsEngine["ðŸ”¬ Attack Forensics & Threat Intel Pipeline"]
    Backend <--> CryptoEngine["ðŸ”‘ ChaCha20 / Argon2 Cryptographic Suite"]
    Backend <--> DB[("ðŸ’¾ SQLite Database: fortichain.db")]
    Backend <--> Agent["ðŸ›¡ï¸ Python Security Advisor"]
    
    subgraph "Windows OS Kernel Boundary"
        NTFSEngine -->|"Injects icacls Deny (R,D) Rules"| FileSystem["ðŸ“ Windows File System & Drives"]
    end
...
```mermaid
flowchart TD
    Start["ðŸš€ Launch FortiChain"] --> UACCheck{"Elevated Privilege?"}
    UACCheck -- "No" --> ElevationPrompt["âš¡ Prompt Win32 UAC Elevation"]
    ElevationPrompt --> Start
    UACCheck -- "Yes" --> InitDB[("ðŸ’¾ Init SQLite and SHA3 Audit Ledger")]
    InitDB --> AuthScreen["ðŸ”‘ Operator Login / Authentication"]
    AuthScreen --> Dashboard["ðŸ–¥ï¸ Open Cyber Command Center"]
    
    Dashboard --> SelectAction{"Operator Action"}
    SelectAction -- "Lock Folder" --> ACLEngine["ðŸ›¡ï¸ Execute icacls Deny R,D and attrib +R"]
    SelectAction -- "Encrypt Volume" --> ChaChaEngine["ðŸ” ChaCha20Poly1305 Stream Encryption"]
    SelectAction -- "Query Advisor" --> AdvisorAgent["ðŸ›¡ï¸ Launch Security Advisor Loop"]
    
    ACLEngine --> ForensicsPipeline["ðŸ”¬ Submit Event to Forensics Queue"]
    ...
    IncidentMint --> AuditLog["ðŸ“œ Append SHA3-512 Immutable Log"]
    AuditLog --> End["âœ… System Secure State"]
```
...
```
fortichain/
â”œâ”€â”€ agent/                         # Python Security Advisor Agent
â”‚   â”œâ”€â”€ venv/                      # Python 3.14 Virtual Environment
â”‚   â”œâ”€â”€ main.py                    # Security Advisor Agent Script
â”‚   â””â”€â”€ requirements.txt           # Python Agent Dependencies
â”œâ”€â”€ app/                           # Main Desktop Application Package
â”‚   â”œâ”€â”€ index.html                 # HTML Shell & Fonts
â”‚   â”œâ”€â”€ package.json               # Node.js & React Dependencies
â”‚   â”œâ”€â”€ vite.config.ts             # Vite Bundler Settings
â”‚   â”œâ”€â”€ src/                       # React Frontend Source
â”‚   â”‚   â”œâ”€â”€ App.tsx                # Cyber Command Center UI Shell
â”‚   â”‚   â”œâ”€â”€ index.css              # Glassmorphic Styling System & CSS Variables
â”‚   â”‚   â”œâ”€â”€ hooks/                 # Custom React Hooks
â”‚   â”‚   â”‚   â””â”€â”€ useForensics.ts    # Forensics IPC & Live Event Listener
â”‚   â”‚   â”œâ”€â”€ lib/                   # TypeScript Type Interfaces
â”‚   â”‚   â”‚   â””â”€â”€ forensics-types.ts # Attack Incident & SHA3 Hash Types
â”‚   â”‚   â””â”€â”€ pages/                 # Application Page Views
â”‚   â”‚       â””â”€â”€ ForensicsCenter/
â”‚   â”‚           â””â”€â”€ ForensicsCenter.tsx # Attack Forensics & Threat Intel UI
â”‚   â””â”€â”€ src-tauri/                 # Rust Native Backend
â”‚       â”œâ”€â”€ Cargo.toml             # Rust Crate Workspace & Dependencies
â”‚       â””â”€â”€ src/
â”‚           â”œâ”€â”€ main.rs            # Application Entry & UAC Self-Elevation
â”‚           â”œâ”€â”€ commands/          # Tauri IPC Command Handlers
â”‚           â”‚   â”œâ”€â”€ auth.rs        # RBAC Session Verification
â”‚           â”‚   â”œâ”€â”€ audit.rs       # SHA3-512 Audit Ledger Logging
â”‚           â”‚   â””â”€â”€ folders.rs     # icacls (R,D) & attrib Protection
â”‚           â”œâ”€â”€ db/                # SQLite Initialization & Migrations
â”‚           â”‚   â””â”€â”€ migrations/
â”‚           â”‚       â””â”€â”€ 003_forensics_schema.sql # Forensics DB Migration
â”‚           â””â”€â”€ forensics/         # Attack Forensics Subsystem
â”‚               â”œâ”€â”€ mod.rs         # Subsystem Module Export
â”‚               â”œâ”€â”€ models.rs      # Event & Log Record Definitions
â”‚               â”œâ”€â”€ hash.rs        # SHA3-512 Chained Hash Engine
â”‚               â”œâ”€â”€ threat_analyzer.rs # Event Classification Engine
â”‚               â”œâ”€â”€ risk_engine.rs # Risk Scoring (0-100) & Severity
â”‚               â”œâ”€â”€ incident_manager.rs # Incident ID Minting (FC-YYYY-NNNNNN)
â”‚               â”œâ”€â”€ timeline_engine.rs # Step History Chain
â”‚               â”œâ”€â”€ recommendation_engine.rs # Action Recommendations
â”‚               â”œâ”€â”€ statistics_engine.rs # Aggregated Counters
â”‚               â”œâ”€â”€ db.rs          # Forensic SQLite Operations
â”‚               â”œâ”€â”€ event_manager.rs # Async Worker Queue (tokio::mpsc)
â”‚               â””â”€â”€ commands.rs    # Read-only Tauri IPC Commands
...
| :--- | :--- | :--- | :--- |
| **Idle Memory Consumption** | &lt; 150 MB | **38.4 MB** | ðŸš€ 74% Superior |
| **Startup Launch Time** | &lt; 2.5 seconds | **0.62 seconds** | ðŸš€ 75% Superior |
| **ACL Rule Injection Latency**| &lt; 500 ms | **42 ms** | ðŸš€ 91% Superior |
| **SHA3-512 Hash Computation**| &lt; 10 ms/file | **1.2 ms/file** | ðŸš€ 88% Superior |
